  ...
  <body>
	<?php
		...
		*/
		...
		$profe_send = $_REQUEST['profe_send'];
		
		
		$registrosProfe=mysqli_query($conexion,"select * from profesionalismo where id_profe != $profe_send")
    ...
    <div id="main" class="grupo">
      ...
      <div id="content" class="caja web-65">
        <div id="registro_publicaciones">
          ...
          <ul class="anteriores">
			<?php
				while($reg=mysqli_fetch_array($registrosProfe)){					
        ...
        <div class="caja web-25">
          <form method="post" id="footer" class="right">
			...
			<?php
				...
				echo "<input type=\"text\" name=\"profe_send\" value=\"$profe_send\" hidden=hidden>";	
			?>
          </form>
        </div>
      </div>
