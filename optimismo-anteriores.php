  ...
  <body>
	<?php
		...
		*/
		...
		$opti_send = $_REQUEST['opti_send'];
		
		
		$registrosOptimismo=mysqli_query($conexion,"select * from optimismo where id_opti != $opti_send")
          ...
          <ul class="anteriores">
			<?php
				while($reg=mysqli_fetch_array($registrosOptimismo)){					
					...
					$id_opti = $reg['id_opti'];					
					echo "<li><a href=\"optimismo.php?id_opti=",urlencode($id_opti)," \">$titulo_opti</a></li>";										
        ...
        <div class="caja web-25">
          <form method="post" id="footer" class="right">
			...
			<?php
				...
				echo "<input type=\"text\" name=\"opti_send\" value=\"$opti_send\" hidden=hidden>";	
			?>
          </form>
        </div>
      </div>
