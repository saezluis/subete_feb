...
<html lang="es">
  <head>
    <title>Productividad / Súbete</title>
    <meta charset="utf-8">
  ...
  <body>
	<?php
		...
		if(@$get_activo==true){
			$registros_produ = mysqli_query($conexion,"SELECT * FROM produ WHERE id_produ = '$id_produ'")
			or die("Problemas en el select:".mysqli_error($conexion));
		}else{
			$registros_produ = mysqli_query($conexion,"SELECT * FROM produ WHERE id_produ = (SELECT MAX(id_produ) FROM produ)")
			or die("Problemas en el select:".mysqli_error($conexion));
		}
		
		if($reg=mysqli_fetch_array($registros_produ)){
		
			$id_produ = $reg['id_produ'];
			$imagen_produ = $reg['imagen_produ'];
